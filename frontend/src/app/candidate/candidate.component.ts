...
export class CandidateComponent implements OnInit, OnDestroy {
  ...
  onNewCandidate() {
    if (!this.editJobMode) {
      ...
      let newLanguages = this.newCandidateForm.value.languages.split(',');
      let newCertifications = this.newCandidateForm.value.certifications.split(',');
      let newExperience = this.newCandidateForm.value.experience.split(',');
      ...
      if (this.uploadImgForm.value.image !== null) {
        this.docsService
          .addDoc(
            ...
            this.newCandidateForm.value.nic,
      ...
      this.newCandidateForm.value.skills += '';
      this.newCandidateForm.value.languages += '';
      this.newCandidateForm.value.certifications += '';
      this.newCandidateForm.value.experience += '';
  ...
  async onViewDetails(candidate: any) {
    ...
    this.toViewCandidate.nic = candidate.nic;
    ...
    this.toViewCandidate.languages = candidate.languages;
    this.toViewCandidate.certifications = candidate.certifications;
    this.toViewCandidate.experience = candidate.experience;
  ...
  exportData(){
    const data = [];
    this.rowData2.forEach((candidate) => {
      const {fullName, email, phone} = candidate;
      data.push([fullName, email, phone])
    })
    ...
    autoTable(doc, {
      ...
      didDrawPage: function (_data) {
        doc.setFontSize(20);
        doc.setTextColor(40);
        doc.text(`Job Report | (${data.length}) elements`, _data.settings.margin.left + 15, 10);
      }
    });

    doc.text(`Candidates Report | (${data.length}) elements`, 15, 10);
