...
export class SearchComponent implements OnInit, OnDestroy {
  ...
  exportData() {
    ...
    this.rowData.forEach((job) => {
      const { title, team, createdAt, skills, description } = job;
      data.push([title, team, createdAt, skills.join(", "), description]);
    });

    const doc = new jsPDF();
    autoTable(doc, {
      head: [["Title", "Company", "Date", "Skills", "Description"]],
      body: [...data],
